...
import { BiHome, BiTask, BiChevronLeft, BiChevronRight, BiTrain, BiLogOut, BiCurrentLocation } from "react-icons/bi";
import { Link, useLocation, useNavigate } from 'react-router-dom';
import { useAuth } from '../context/AuthContext';
import { useSidebar, getNavPillClass, getNavIconClass } from '../context/SidebarContext';

const navItems = [
  { to: '/UserDashboard', icon: BiHome, label: 'Dashboard' },
  { to: '/taskManager', icon: BiTask, label: 'Tasks' },
  { to: '/training', icon: BiTrain, label: 'Training' },
  { to: '/Geolocation Interface', icon: BiCurrentLocation, label: 'GeoLocation' },
];
  ...
  return (
    ...
    >
      <div className={`flex items-center ${collapsed ? 'justify-center' : 'justify-between'}`}>
        {!collapsed && (
          <p className="text-xs uppercase tracking-[0.4em] text-slate-400">Worker</p>
        )}
        <button
          onClick={toggle}
